// src/components/ExploreOriginal/ExplorePremium.tsx
import React, {
  useEffect,
  useRef,
  useState,
...
type Tab = { label: string; dropdown: boolean };
...
/* ✅ 외부 검색 링크(네이버 카페만) */
const buildCafeUrl = (region?: string, sub?: string, title?: string): string => {
  const parts: string[] = ["서산"];
  if (region && region !== "전체") parts.push(region);
  if (sub) parts.push(sub);
  if (title) parts.push(title);
  const q = parts.join(" ");
  return `https://search.naver.com/search.naver?where=article&query=${encodeURIComponent(
    q
  )}`;
};
          ...
          <div className={styles.infoPanel}>
            <div className={styles.panelBanner}>
              <img
                src={sparkleIcon}
                alt=""
                className={styles.bannerSparkle}
              />
...
/* ===== 메인(목록 + 상세 전환) ===== */
export default function ExplorePremium(): JSX.Element {
  ...
  const [activeSub, setActiveSub] = useState<string>(subFromUrl || "");
  ...
  useEffect(() => {
    if (regionFromUrl && REGIONS.includes(regionFromUrl as Region))
      setActiveRegion(regionFromUrl as Region);
  ...
  const countText = useMemo(
    () => `결과 ${filteredItems.length.toLocaleString()}개`,
              ...
              {t.dropdown && opened && (
                <div className={styles.ddMenu}>
                  <ul className={styles.ddList}>
                    {DROPDOWN[t.label as keyof typeof DROPDOWN].map(
                      (opt, idx) => (
                        <li
                          key={opt}
